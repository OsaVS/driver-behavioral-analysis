...
void setup() {
  ...
  // Configure filters for extended IDs (accept 0x18DAF1XX responses)
  mcp2515.setFilterMask(MCP2515::MASK0, true, 0x1FFFFF00);  // Mask to match 0x18DAF1XX
  mcp2515.setFilter(MCP2515::RXF0, true, 0x18DAF110);       // Accept 0x18DAF110 (ECU #0)
  mcp2515.setFilter(MCP2515::RXF1, true, 0x18DAF110);       // Duplicate for buffer 1
...
void readResponse() {
  ...
  while (millis() < timeout) {
    if (mcp2515.readMessage(&canMsgRecv) == MCP2515::ERROR_OK) {
      if ((canMsgRecv.can_id & 0x1FFFFFFF) == 0x18DAF110) {  // Check extended response ID for ECU #0
        ...
        if (pid == 0x0C) {  // RPM
          ...
          Serial.print("Negative response, error code: ");
